(() => {
    ...
    function initSliders() {
        if (document.querySelector(".mainSwiper__images") && document.querySelector(".thumbSwiper__images")) {
            var thumbSwiper = new Swiper(".thumbSwiper__images", {
                ...
                watchSlidesProgress: true,
                navigation: {
                    nextEl: ".swiper-button-next__images",
                    prevEl: ".swiper-button-prev__images"
                },
                pagination: {
                    el: ".swiper-pagination__images",
                    clickable: true
                }
            });
            var mainSwiper = new Swiper(".mainSwiper__images", {
        ...
        if (document.querySelector(".mainSwiper__video") && document.querySelector(".thumbSwiper__video")) {
            thumbSwiper = new Swiper(".thumbSwiper__video", {
                modules: [ Navigation, Thumb, Pagination ],
                loop: false,
                spaceBetween: 12,
                slidesPerView: "auto",
                ...
                watchSlidesProgress: true,
                navigation: {
                    nextEl: ".swiper-button-next__video",
                    prevEl: ".swiper-button-prev__video"
                },
                pagination: {
            ...
            mainSwiper = new Swiper(".mainSwiper__video", {
                modules: [ Navigation, Thumb ],
                loop: true,
                spaceBetween: 8,
                thumbs: {
        ...
        if (document.querySelector(".landing-vslider__main") && document.querySelector(".landing-vslider__content")) {
            thumbSwiper = new Swiper(".landing-vslider__content", {
                modules: [ Navigation, Thumb, Pagination ],
                direction: "vertical",
                loop: false,
                spaceBetween: 0,
            ...
            function updateThumbClasses() {
                document.querySelectorAll(".landing-vslider__content .swiper-slide").forEach((slide => {
                    slide.classList.remove("thumb-prev", "thumb-active", "thumb-next");
                }));
                let activeIndex = thumbSwiper.activeIndex;
                thumbSwiper.slides[activeIndex]?.classList.add("thumb-active");
                thumbSwiper.slides[activeIndex - 1]?.classList.add("thumb-prev");
                thumbSwiper.slides[activeIndex + 1]?.classList.add("thumb-next");
    ...
    function addClassOnVisible(selector, className, delay = 100, rootMargin = "-50px") {
    ...
    document.addEventListener("DOMContentLoaded", (function() {
        ...
        let currentIndex = -1;
        const prevButton = document.querySelector(".popup__nav._prev");
        const nextButton = document.querySelector(".popup__nav._next");
        if (!prevButton || !nextButton) {
            console.error("Не найдены кнопки переключения (prev, next)");
            return;
        }
        function showMediaByIndex(index) {
            if (index < 0 || index >= mediaItems.length) {
                console.error("Индекс выходит за пределы диапазона");
                return;
            }
            ...
            if (!mediaContainer) {
                console.error("Не найден контейнер для медиа");
                return;
            }
            ...
            } else console.error("Нет данных для изображения или видео");
        }
        ...
        function handleGodsItems() {
            ...
            if (godsItems.length > 0) godsItems.forEach((item => {
                ...
                if (content && text) {
                    ...
                    } else item.addEventListener("click", (function() {
                        if (content.style.bottom === "0px") content.style.bottom = `-${textHeight}px`; else content.style.bottom = "0";
                    }));
                }
            }));
        }
